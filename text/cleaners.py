...
def zh_en_ja_mixture_cleaners(text):
    # 这部分未完成，所以用不了。

    chinese_texts = regex.findall(r'\p{Script=Han}+', text)
    japanese_texts = regex.findall(r'\p{Script=Hiragana}+|\p{Script=Katakana}+', text)
    # english_texts = 

    for chinese_text in chinese_texts:
        cleaned_text = chinese_cleaners(chinese_text)
...
def zh_ja_mixture_cleaners(text):
    ...
    for chinese_text in chinese_texts:
        ...
        text = text.replace(chinese_text, cleaned_text+' ', 1)
    for t in single_en:
        cleaned_text = chinese_cleaners(t)
        text = text.replace(t, cleaned_text+' ', 1)
    for japanese_text in japanese_texts:
        cleaned_text = japanese_cleaners2(
            japanese_text).replace('ts', 'ʦ').replace('u', 'ɯ').replace('...', '…')
        text = text.replace(japanese_text, cleaned_text+' ', 1)
